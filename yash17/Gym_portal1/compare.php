...
//average raging of 1st gym
 ...
 $ratingDetails1 = "SELECT ratingNumber FROM item_rating  where gym_id=(select gym_id from gyms where gym_id='$productTwo' )";
	$rateResult1 = mysqli_query($con, $ratingDetails1) or die("database error:". mysqli_error($con));
	$ratingNumber1 = 0;
	$count1 = 0;
	$fiveStarRating1 = 0;
	$fourStarRating1 = 0;
	...
	$twoStarRating1 = 0;
	$oneStarRating1 = 0;
	while($rate1 = mysqli_fetch_assoc($rateResult1)) {
		$ratingNumber1+= $rate1['ratingNumber'];
		$count1 += 1;
		if($rate1['ratingNumber'] == 5) {
			$fiveStarRating1 +=1;
		} else if($rate1['ratingNumber'] == 4) {
			$fourStarRating1 +=1;
		} else if($rate1['ratingNumber'] == 3) {
			$threeStarRating1 +=1;
		} else if($rate1['ratingNumber'] == 2) {
			$twoStarRating1 +=1;
		} else if($rate1['ratingNumber'] == 1) {
			$oneStarRating1 +=1;
		}
	}
	$average1 = 0;
	if($ratingNumber1 && $count1) {
		$average1 = $ratingNumber1/$count1;
	}     
        $averageRating1 = round(2*$average1)/2; 
  	...
  	<div class="container">
        ...
        <div class="col-sm-6" style="margin-top: 30px;">
          ...
          <div class="card">
            ...
            <p><b>Description</b>.<?php echo $row1['gym_desc']; ?></p>
              <p><b>Number of trainers:  </b>
             <?php
             if($row2['cnt']<$row1['cnt'])
                 { 
                    echo "<span style='color:green;font-weight:bold'>".$row1['cnt']."</span>";                
                 } 
                 elseif($row2['cnt']>$row1['cnt'])
                 {
                     echo "<span style='color:red;font-weight:bold'>".$row1['cnt']."</span>";   
                 }
                 else
                 {
                     echo "<span style='color:blue;font-weight:bold'>".$row1['cnt']."</span>";   
                 }
                 ?></p>
         <!--  <p><b>Rating:  </b><span style="color:blue;font-weight:bold"><?php echo $averageRating; ?></span></p> -->
           <p><b>Rating:  </b><?php if($averageRating<$averageRating1)
               { 
               echo "<span style='color:red;font-weight:bold'>".$averageRating;
               }
               elseif($averageRating>$averageRating1)
                   {
                   echo "<span style='color:green;font-weight:bold'>".$averageRating;                  
                   }
                   else
                   {
                      echo "<span style='color:blue;font-weight:bold'>".$averageRating; 
                   }
                   ?></span></p> 
              <p><b>Plans: </b></p>
              <table>
              ...
              while ($row = mysqli_fetch_assoc($plan_query)){
                ?>
                  <tr>
                            ...
                            <td> <span style="color:#008B8B;font-weight:bold">  <?php echo $row['plan_price']; ?></span></td>
                  </tr>   
                 <?php
              }
              ?>
        ...
        <div class="col-sm-6" style="margin-bottom: 30px; padding: 5px;">
         ...
         <p><b>Number of trainers:  </b>
             ...
             if($row2['cnt']>$row1['cnt'])
                 { 
                    echo "<span style='color:green;font-weight:bold'>".$row2['cnt']."</span>";                
                 } 
                 elseif($row2['cnt']<$row1['cnt'])
                 {
                     echo "<span style='color:red;font-weight:bold'>".$row2['cnt']."</span>";   
                 }
                 else
                 {
                     echo "<span style='color:blue;font-weight:bold'>".$row2['cnt']."</span>";   
                 }
                 ?></p>
   <!--      <p><b>Rating:  </b><span style="color:blue;font-weight:bold"><?php echo  "<span style='color:red;font-weight:bold'>".$averageRating; ?></span></p> -->
          <p><b>Rating:  </b><?php
          if($averageRating>$averageRating1)
              { 
              echo "<span style='color:red;font-weight:bold'>".$averageRating1;              
              }
              elseif ($averageRating>$averageRating1) 
              {
                  echo "<span style='color:green;font-weight:bold'>".$averageRating1;
              }
              else
              {
                  echo "<span style='color:blue;font-weight:bold'>".$averageRating1;              
              }
              ?></span></p> 
